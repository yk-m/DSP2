...
Array_d* cloneArray_d( Array_d* vector, int dim ) {
	...
	copyArray_d( vector, new_vector );
	return new_vector;
}

void copyArray_d( Array_d* vector, Array_d* to_vector ) {
	int i;

	for ( i = 0; i < vector->length && i < to_vector->length; i++ ) {
		to_vector->a[i] = vector->a[i];
	}
	for ( ; i < to_vector->length; i++ ) {
		to_vector->a[i] = 0;
	}
}

void swapArray_d(Array_d **a, Array_d **b)
{
	Array_d *tmp;
...
Array_d* splitArray_d( Array_d* a, int start, int end ) {
	Array_d* new_array = newArray_d( end - start );
	for ( int i = 0; i < new_array->length; ++i ) {
		new_array->a[i] = a->a[i+start];
	}
	return new_array;
}
